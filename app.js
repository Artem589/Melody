...
arr.forEach(i => {
    ...
    counter.textContent = e.target.getAttribute('data-floor') // записывает значение атрибута внутрь счетчика
    })
})
...
btnUp.addEventListener("click", (e) => {
  if (currentFloor < 18) {
    // Если число меньше 18 этажей , то выполнить следующее
    currentFloor++; // прибавлять +1 к каждому числу
    usCurrent = currentFloor.toLocaleString("en-Us", {
      minimumIntegerDigits: 2,
      useGrouping: false,
    }); //генерируем 2 значное число(02)
    counter.textContent = usCurrent; // записываем значение внутрь счётчика
  }
